...
1. Read today's brief markdown + charts from ``output/briefs`` /
   ``output/charts`` (already produced by ``altdata-brief generate``).
...
class GhPagesPublisher:
    ...
    def plan(self, date_str: str) -> PublishPlan:
        ...
        if not brief_path.exists():
            raise PublishError(
                ...
                "(run `uv run altdata-brief generate --date "
                f"{date_str}` first)"
            )
    ...
    def publish(
        ...
        *,
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="altdata-publish-") as tmp:
...
def _render_index_md(
    ...
    *,
    ...
    lines.append("    }")
    lines.append("  }")
    lines.append("  if (!/^https?:$/.test(window.location.protocol)) {")
    lines.append('    setStatus("本地预览 · 自动刷新关闭", "");')
    lines.append("    return;")
    lines.append("  }")
    ...
    lines.append("---")
    lines.append("")
    lines.append("由 [`altdata-brief`](https://github.com/Leonard-Don/altdata-brief) 生成 · MIT")
    lines.append("")
    return "\n".join(lines)
...
def default_template_dir() -> Path:
    ...
    # publish/gh_pages.py → src/altdata_brief/publish/ → src/altdata_brief/
    # → src/ → <repo_root>
