...
#: Public-summary schema versions this adapter can parse.
#:
#: ETF 512400 is special: ``liveSnapshot.json`` is regenerated by a JS app
#: that predates the ``schema_version`` convention and does NOT emit the
#: field. ``implicit_version=1`` makes that absence an *explicit, declared*
#: fallback rather than an accidental mis-parse — see
#: :class:`~altdata_brief.adapters.schema.SchemaContract`. When the JS
#: app eventually starts emitting ``schema_version``, an unrecognized value
#: still raises loudly; only the documented "missing" case maps to v1.
_SCHEMA = SchemaContract(
    source="etf_512400", supported=frozenset({1}), implicit_version=1
)
...
class ETF512400Adapter(AdapterBase):
    ...
    def _load_from_public_summary(
        ...
        """Read the snapshot via the public-summary contract.

        The on-disk artifact is the same file :meth:`fetch_cached` reads,
        but this path reports ``source_mode="public"`` so the validate
        layer can distinguish "we honored ``--source-mode public``" from
        "we fell back to cache".

        ``schema_version`` is resolved against :data:`_SCHEMA` before
        parsing. ``liveSnapshot.json`` does not carry the field today, so
        its absence maps to the declared ``implicit_version`` (1) — an
        explicit, documented decision. If a future JS-app revision starts
        emitting ``schema_version`` with an unknown value, that raises
        ...
        if version == 1:
            ...
            raise AssertionError(f"unhandled etf_512400 schema_version={version}")
