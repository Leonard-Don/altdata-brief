...
1. **Live endpoint** — ``SUPER_PRICING_LIVE_API`` (or legacy
   ``ALTDATA_BRIEF_LIVE=1``) → hit ``/api/v1/alt-data/narrative``.
...
from altdata_brief.adapters.base import AdapterBase, AdapterPayload, AdapterUnavailable
from altdata_brief.adapters.schema import SchemaContract, resolve_schema_version
...
class SuperPricingAdapter(AdapterBase):
    ...
    ``ALTDATA_BRIEF_PREFERENCE`` or ``--source-mode``).
    """
    ...
    def _load_from_public_summary(
        ...
        :class:`~altdata_brief.adapters.schema.UnsupportedSchemaVersionError`
