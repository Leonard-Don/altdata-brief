...
def _sdk_module() -> Any | None:
    ...
    monkey-patch ``cn_altdata_brief.llm.anthropic_client._sdk_module``
...
def _build_user_prompt(raw_text: str, context: dict[str, Any]) -> str:
    ...
    *guard-rails* (so the model knows which industry names must
    ...
    return (
        "下面是 cn-altdata-brief 当日「本日观察」段的规则化版本，"
        "请改写为 3 句新闻体中文，不增删事实：\n\n"
        f"```\n{raw_text}\n```"
    ...
    *,
    ...
    if sdk is None:
        return RephraseResult(
            ...
            status="sdk_missing",
            note="anthropic SDK not installed; run `pip install cn-altdata-brief[llm]`",
