...
def test_required_paths_wrong_container_kind_fails_with_path(tmp_path: Path) -> None:
    """A public-summary container must match the adapter's exact access pattern."""
    summary = _write_summary(
        tmp_path / "sp.json",
        {
            "schema_version": 1,
    ...
    entry = r.detail["per_source"]["super_pricing"]
    assert "providers.macro_hf.metals" in entry["missing_paths"]
    assert entry["invalid_type_paths"]["providers.macro_hf.metals"] == "list"
    ...
    summary = _write_summary(
        tmp_path / "sp.json",
        {
            ...
            "providers": {
                ...
                "macro_hf": {"metals": {"copper": {"trend": "up"}}},
            },
        },
    )
...
def test_required_paths_non_numeric_policy_count_fails_with_path(
    tmp_path: Path,
) -> None:
    """policy_count must be usable by the adapter's int() normalization."""
    summary = _write_summary(
        tmp_path / "sp.json",
        {
            "schema_version": 1,
    ...
    entry = r.detail["per_source"]["super_pricing"]
    assert "providers.policy_radar.policy_count" in entry["missing_paths"]
    assert entry["invalid_type_paths"]["providers.policy_radar.policy_count"] == "str"


def test_required_paths_negative_policy_count_fails_with_path(
    tmp_path: Path,
) -> None:
    """policy_count is a count, so negative upstream values are unusable."""
    summary = _write_summary(
        tmp_path / "sp.json",
        {
            "schema_version": 1,
    ...
    entry = r.detail["per_source"]["super_pricing"]
    assert "providers.policy_radar.policy_count" in entry["missing_paths"]
    assert entry["invalid_type_paths"]["providers.policy_radar.policy_count"] == "int"


def test_required_paths_fractional_policy_count_fails_with_path(
    tmp_path: Path,
) -> None:
    """policy_count is a whole-number count; fractional JSON numbers are unusable."""
    summary = _write_summary(
        tmp_path / "sp.json",
        {
            "schema_version": 1,
    ...
    entry = r.detail["per_source"]["super_pricing"]
    assert "providers.policy_radar.policy_count" in entry["missing_paths"]
    assert entry["invalid_type_paths"]["providers.policy_radar.policy_count"] == "float"
